...
class Momentum(Optimizer):
    ...
    def init_state(self, param:Parameter):
        last_step = np.zeros_like(param.grad_array)
        return last_step
    def step(self, param:Parameter,last_step):
...
class RMSProp(Optimizer):
    ...
    def init_state(self, param):
        mean_square = np.zeros_like(param.grad_array)
        return mean_square
    ...
    def step(self, param, mean_square):
        ...
        np.sqrt(mean_square, tmp)
        tmp += self.eps
        np.divide(grad, tmp, tmp)
        tmp *= -self.learn_rate
        param.step(tmp)
