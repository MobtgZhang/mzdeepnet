import numpy as np
from mzdeepnet.base import ParamMixin, PickleMixin
...
class Affine(Layer, ParamMixin):
    ...
    def fprop(self, x):
        self._tmp_x = x
        return np.dot(x, self.weights.array) + self.bias.array

    def bprop(self, y_grad):
        np.dot(self._tmp_x.T, y_grad, out=self.weights.grad_array)
        np.sum(y_grad, axis=0, out=self.bias.grad_array)
        if self.bprop_to_x:
            return np.dot(y_grad, self.weights.array.T)
